...
from src.mailing.notifications.keyboards import periodicity_kb, timezone_kb, all_periods, weekdays_kb, type_translation
...
async def unsubscribe(callback_query: CallbackQuery, bot: Bot):
    ...
    # Приведение времени к формату ЧЧ:ММ и преобразование в time-объект
    try:
        if len(time_str) > 5:
            time_str = time_str[:5]  # Обрезаем до ЧЧ:ММ
    ...
    # Подключение к базе и удаление подписки
    conn = await asyncpg.connect(**DB_CONFIG)
    try:
        ...
        # Попытка удалить связанные задачи из планировщика
        possible_job_ids = [
        ...
        subscription_type_ru = type_translation.get(subscription_type, subscription_type)

        # Ответ пользователю
        await callback_query.message.answer(
            text=f"✅ Вы успешно отменили подписку <b>{subscription_type_ru}</b> в <b>{time_str}</b>.",
            reply_markup=get_main_menu_keyboard(),
...
async def manage_subscription(callback_query: CallbackQuery):
    ...
    await callback_query.message.answer(f"Вы выбрали {subscription_type_ru} подписку на время: <b>{time}</b>.", parse_mode=ParseMode.HTML)
