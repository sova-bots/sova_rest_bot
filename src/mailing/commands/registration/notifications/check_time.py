...
async def send_report(
    ...
    try:
        ...
        # Если запрошен формат рекомендаций
        if format_type == "recommendations":
            if report_type == "revenue":
                if revenue_recommendations:
                    ...
                    await bot.send_message(
                        user_id,
                        header + "Для данного типа revenue-отчёта рекомендации не найдены.",
                        parse_mode="HTML",
                        reply_markup=get_main_menu_keyboard()
                    )
                    return
        ...
        for i, text in enumerate(texts):
            ...
            if i == 0:
                # Проверяем, содержит ли текст заголовок
                if any(tag in text for tag in ["📍", "📊", "📅", "<b>Объект", "<code>Объект"]):
                    # Заголовок уже в тексте — не добавляем header
                    await bot.send_message(
                        user_id,
                        text,
                ...
                # Остальные части без проверки — просто отправляем
                await bot.send_message(
                    user_id,
                    text,
                    parse_mode=parse_mode
        ...
        # Добавляем кнопку после отправки всех частей отчета
        await bot.send_message(
            user_id,
